...
- The README is **partly machine-written**. Three GitHub Actions inject content into it on a schedule, each bounded by HTML comment markers that must survive any edit:
  - `<!-- BLOG-POST-LIST:START -->` / `<!-- BLOG-POST-LIST:END -->` — `gautamkrishnar/blog-post-workflow`, pulls 10 posts from the site RSS nightly.
  - `<!--START_SECTION:waka-->` / `<!--END_SECTION:waka-->` — `anmol098/waka-readme-stats`, nightly.
  - `<!--START_SECTION:activity-->` / `<!--END_SECTION:activity-->` — `jamesgeorge007/github-activity-readme`, every 30 minutes.
- Changing prose, ordering, or section structure is safe. Removing or renaming a marker pair is not: the corresponding workflow stops updating and the README silently goes stale.
